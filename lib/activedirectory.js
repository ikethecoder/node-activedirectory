...
defaultAttributes = originalDefaultAttributes = {
  user: [ 
    ...
    'comment', 'description', 'objectClass'
  ],
  group: [
    'dn', 'cn', 'description', 'objectClass'
  ]
};
...
function getUserQueryFilter(username) {
  ...
  if (! username) return('(objectClass=posixAccount)');
  if (isDistinguishedName.call(self, username)) {
    return('(&(objectClass=posixAccount)(distinguishedName='+parseDistinguishedName(username)+'))');
  }

  return('(&(objectClass=posixAccount)(|(uid='+username+')(sAMAccountName='+username+')(userPrincipalName='+username+')))');
}
...
function getGroupQueryFilter(groupName) {
  ...
  if (! groupName) return('(objectClass=posixGroup)');
  if (isDistinguishedName.call(self, groupName)) {
    return('(&(objectClass=posixGroup)(distinguishedName='+parseDistinguishedName(groupName)+'))');
  }
  return('(&(objectClass=posixGroup)(cn='+groupName+'))');
}
...
function isGroupResult(item) {
  ...
  if ((item.objectClass) && (item.objectClass.length > 0)) {
    return(_.any(item.objectClass, function(c) { return(c.toLowerCase() === 'posixgroup'); }));
  }
  return(false);
}
...
function isUserResult(item) {
  ...
  if ((item.objectClass) && (item.objectClass.length > 0)) {
    return(_.any(item.objectClass, function(c) { return(c.toLowerCase() === 'uid'); }));
  }
  return(false);
}
...
function getRequiredLdapAttributesForGroup(opts) {
  ...
  return(_.union([ 'dn', 'objectCategory', 'groupType', 'cn' ], 
                 includeGroupMembershipFor(opts, 'group') ? [ 'member', 'memberUid' ] : []));
}
...
function getRequiredLdapAttributesForUser(opts) {
  ...
  return(_.union([ 'dn', 'cn' ], 
                 includeGroupMembershipFor(opts, 'user') ? [ 'member', 'memberUid' ] : []));
}
...
ActiveDirectory.prototype.getUsersForGroup = function getUsersForGroup(opts, groupName, callback) {
  ...
  self.findGroup(_.defaults({}, _.omit(opts || {}, 'attributes'), {
                   attributes: joinAttributes((opts || {}).attributes || defaultAttributes.group, [ 'member', 'memberUid' ])
                 }), 
    ...
    var memid = ('member' in group) ? 'member':'memberUid';

    // If only one result found, encapsulate result into array.
    if (typeof(group[memid]) === 'string') {
      group[memid] = [ group[memid] ];
    }
    ...
    var chunks = chunk(group[memid] || [], defaultPageSize);
    if (chunks.length > 1) {
      log.debug('Splitting %d member(s) of "%s" into %d parallel chunks',
                (group[memid] || []).length, groupName, chunks.length);
    }
    ...
    async.each(chunks, function getUsersForGroup_ChunkItem(members, asyncChunkCallback) {
      ...
      var filter = _.reduce(members || [], function(memo, member, index) {
        return(memo+'(uid='+parseDistinguishedName(member)+')');
      }, '');
      filter = '(&(|(objectCategory=posixAccount))(|'+filter+'))';
      // Ignore filter ((objectCategory=posixGroup)) because we assume membership is only users (uid)
      var localOpts = {
        filter: filter,
        scope: 'sub',
